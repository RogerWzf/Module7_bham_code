# Adapted the code from https://github.com/workflow4metabolomics/anova/tree/master/galaxy/stat_anova

# function avova
anova = function (file, sampleinfo, varinfo, mode="row", condition=1, 
                  interaction=F, method="BH", threshold=0.05, 
                  selection_method="intersection", sep="\t", dec=".", 
                  outputdatapvalue="anova.data.output", outputdatasignif="anova.datasignif.output") {

  # Decide whether the factors are factorial or nested
  anova_formula_operator = "+"
  if (interaction) anova_formula_operator = "*"

  # Import the peaks intensity matrix, corresponding sample metadata
  # and variable data
  data <- read.table(file, header = TRUE, row.names=1, sep = sep, quote="\"", 
                  dec = dec, fill = TRUE, comment.char="",na.strings = "NA", check.names=FALSE)
  
  if (mode == "column") {data=t(data)} else {data=as.matrix(data)}
  ...
  if (sum(is.na(match_data_sampleinfoTab)) > 0) {
    write("ERROR: There is a problem during to match sample names from the data 
          matrix and from the sample info (presence of NA).", stderr())
  ...
  # Perform the multiple factor anova
